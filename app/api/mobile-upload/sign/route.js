...
export async function POST(request) {
  try {
    ...
    const verified = verifyUploadToken(body.token);
    if (verified.mode !== "invoice") {
      return NextResponse.json({ error: "This QR code is for batch upload. Use an invoice upload QR code for multi-page invoices." }, { status: 403 });
    }
    ...
    console.error("Signed phone invoice upload failed", {
