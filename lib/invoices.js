...
export async function upsertParsedInvoice({ file, fileBuffer, fileHash, mimeType, ocrResult, parsed }) {
...
export async function upsertParsedInvoiceFromUpload({ displayFileName, primaryFileBuffer, fileHash, mimeType, ocrResult, parsed, storageFiles }) {
  const supabase = getSupabaseAdmin();
...
export async function createPendingInvoiceUpload({ displayFileName, fileHash, mimeType, totalBytes, storageFiles }) {
  const supabase = getSupabaseAdmin();
  const bucket = getStorageBucket();
  const organizationId = await getActiveOrganizationId();
  const uploadGroup = crypto.randomUUID();
...
function storagePathsForInvoice(invoice) {
  const paths = new Set([invoice.original_file_path].filter(Boolean));
  const manifest = parsePendingManifest(invoice.ocr_text);
  for (const pendingFile of manifest.pendingFiles) {
    if (pendingFile?.path) paths.add(pendingFile.path);
  }
