...
import { invoiceExtraStoragePath, invoiceUploadStoragePath, manualInvoiceStoragePath } from "@/lib/storagePaths";
...
export async function createPendingInvoiceUpload({ displayFileName, fileHash, mimeType, totalBytes, storageFiles, organizationId: providedOrganizationId = null }) {
  ...
  const invoiceInsert = await supabase
    ...
    .single();
  if (invoiceInsert.error) throw invoiceInsert.error;
  return { invoiceId: invoiceInsert.data.id, duplicate: false, queued: true };
}

export async function processPendingInvoice(invoiceId, { force = false, organizationId: providedOrganizationId = null } = {}) {
  const supabase = getSupabaseAdmin();
  const bucket = getStorageBucket();
  const organizationId = providedOrganizationId || await getActiveOrganizationId();
  const { data: invoice, error } = await supabase
    ...
    .single();
  if (error) throw error;
  if (!force && !["queued", "processing", "processing_failed"].includes(invoice.parse_status)) {
    return { invoiceId, status: invoice.parse_status, alreadyProcessed: true };
  }

  const manifest = parsePendingManifest(invoice.ocr_text);
  const filesToProcess = force ? originalFileEntries(invoice) : manifest.pendingFiles;
  if (!filesToProcess.length) throw new Error("This invoice does not have original files to process.");

  try {
    if (force || invoice.parse_status !== "processing") {
      let claim = supabase
        .from("invoices")
        .update({ parse_status: "processing" })
        .eq("id", invoiceId)
        .eq("organization_id", organizationId);
      if (!force) claim = claim.eq("parse_status", invoice.parse_status);
      const processingUpdate = await claim.select("id").maybeSingle();
      if (processingUpdate.error) throw processingUpdate.error;
      if (!processingUpdate.data) {
        return { invoiceId, status: "processing", alreadyClaimed: true };
      }
    }

    const ocrResults = await Promise.all(filesToProcess.map(async (pendingFile, index) => {
      ...
      const result = await runOcr({ fileBuffer, mimeType: pendingFile.mimeType, maxPages: 30 });
      return { ...result, fileName: pendingFile.fileName, fileIndex: index + 1 };
    }));
    ...
    if (duplicate) {
      const duplicateUpdate = await supabase
        ...
        .update({
          ...
          invoice_date: parsed.invoice_date,
          invoice_total: parsed.invoice_total,
          ocr_text: serializeProcessedOcr({ text: ocrResult.text, files: filesToProcess }),
          ocr_provider: ocrResult.provider,
    ...
    const failedUpdate = await supabase
      ...
      .update({
        ...
        ocr_text: force
          ? serializeProcessedOcr({ text: ocrTextForInvoice(invoice), files: filesToProcess, error: processingError.message })
          : JSON.stringify({ ...manifest, error: processingError.message })
      })
      .eq("id", invoiceId)
      .eq("organization_id", organizationId);
...
export async function processNextQueuedInvoices({ limit = 1 } = {}) {
  const supabase = getSupabaseAdmin();
  const count = Math.min(Math.max(Number(limit) || 1, 1), 10);
  const { data, error } = await supabase
    .from("invoices")
    .select("id, organization_id, created_at, organizations(subscription_plan, subscription_status)")
  ...
  const jobs = (data || [])
    .sort((a, b) => queuePriority(b) - queuePriority(a) || new Date(a.created_at) - new Date(b.created_at))
    .slice(0, count);
  const results = [];
  for (const job of jobs) {
    try {
      const result = await processPendingInvoice(job.id, { organizationId: job.organization_id });
      results.push({ invoiceId: job.id, ok: true, ...result });
    } catch (error) {
      results.push({ invoiceId: job.id, ok: false, error: error.message });
    }
  }
...
export async function appendInvoiceOriginalFiles(invoiceId, files) {
  ...
  if (prepared.length > 30) throw Object.assign(new Error("Add up to 30 original pages at a time."), { status: 400 });
  ...
  for (const [index, item] of prepared.entries()) {
    const path = invoiceExtraStoragePath({
      organizationId,
      invoiceId,
      uploadGroup,
      index: index + 1,
...
export async function createManualInvoice(body) {
  ...
  const invoiceInsert = await supabase
    ...
    .insert({
      ...
      invoice_date: body.invoice_date,
      invoice_total: num(body.invoice_total),
      original_file_path: manualInvoiceStoragePath({ organizationId, invoiceId: crypto.randomUUID() }),
...
export async function searchLineItems(query, { limit = 1000 } = {}) {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  ...
  const { data, error } = await supabase.rpc("search_invoice_line_items", {
    ...
    result_limit: Math.min(Math.max(Number(limit) || 1000, 1), 10000)
  });
  if (error) throw error;
  return data || [];
}

export async function listLineItems({ limit = 500, offset = 0 } = {}) {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  const pageSize = Math.min(Math.max(Number(limit) || 500, 1), 1000);
...
export async function getDashboardInsights() {
  try {
    ...
    const [thisMonthItems, lastMonthItems, recentUploads, needsReview] = await Promise.all([
      supabase
        .from("invoice_line_items")
        .select("total_cost, invoices!inner(invoice_date, organization_id, duplicate_of_invoice_id, parse_status)")
        .eq("invoices.organization_id", organizationId)
        .is("invoices.duplicate_of_invoice_id", null)
        .neq("invoices.parse_status", "duplicate")
        .gte("invoices.invoice_date", thisMonthStart),
      supabase
        .from("invoice_line_items")
        .select("total_cost, invoices!inner(invoice_date, organization_id, duplicate_of_invoice_id, parse_status, vendors(name))")
        .eq("invoices.organization_id", organizationId)
        .is("invoices.duplicate_of_invoice_id", null)
        .neq("invoices.parse_status", "duplicate")
        .gte("invoices.invoice_date", lastMonthStart)
        .lte("invoices.invoice_date", lastMonthEnd),
      supabase
        .from("invoices")
    ...
    const spendThisMonth = (thisMonthItems.data || []).reduce((sum, item) => sum + (Number(item.total_cost) || 0), 0);
    const spendLastMonth = (lastMonthItems.data || []).reduce((sum, item) => sum + (Number(item.total_cost) || 0), 0);
    const spendTrend = spendLastMonth > 0 ? ((spendThisMonth - spendLastMonth) / spendLastMonth) * 100 : 0;

    const thisMonthVendorItems = await supabase
      .from("invoice_line_items")
      .select("total_cost, invoices!inner(invoice_date, organization_id, duplicate_of_invoice_id, parse_status, vendors(name))")
      .eq("invoices.organization_id", organizationId)
      .is("invoices.duplicate_of_invoice_id", null)
    ...
    const vendorTotals = {};
    for (const item of thisMonthVendorItems.data || []) {
      const name = item.invoices?.vendors?.name || "Unknown";
      vendorTotals[name] = (vendorTotals[name] || 0) + (Number(item.total_cost) || 0);
    }
...
export async function getInvoicesNeedingReview(limit = 200) {
  ...
  const { data, error } = await supabase
    .from("invoices")
    .select("id, invoice_number, invoice_date, parse_status, created_at, original_file_name, invoice_line_items(id), stores(name), vendors(name)")
    .eq("organization_id", organizationId)
    .in("parse_status", ["queued", "processing", "processing_failed", "needs_review"])
    .is("duplicate_of_invoice_id", null)
    .order("created_at", { ascending: false })
    .limit(limit);
  if (error) throw error;
...
function queuePriority(invoice) {
  const status = invoice.organizations?.subscription_status || "active";
  if (!["active", "trialing", "canceling"].includes(status)) return 0;
  const plan = invoice.organizations?.subscription_plan || "free";
  if (plan === "max") return 30;
...
export async function getPriceTracker() {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  const { data, error } = await supabase
    .from("invoice_line_items")
    .select("id, invoice_id, product_name_raw, product_name_normalized, brand, size, unit_cost, invoices!inner(id, invoice_number, invoice_date, organization_id, duplicate_of_invoice_id, parse_status, vendors(name))")
    .eq("invoices.organization_id", organizationId)
    .is("invoices.duplicate_of_invoice_id", null)
    .neq("invoices.parse_status", "duplicate")
  ...
  for (const row of data || []) {
    const key = row.product_name_normalized || row.product_name_raw || "";
    if (!key) continue;
  ...
  for (const [, rows] of groups) {
    const sorted = rows.sort((a, b) => {
      const da = new Date(a.invoices?.invoice_date || 0).getTime();
      const db = new Date(b.invoices?.invoice_date || 0).getTime();
      return da - db;
    });
    const latest = sorted[sorted.length - 1];
    const previous = sorted.length >= 2 ? sorted[sorted.length - 2] : null;
    const first = sorted[0];
    const latestPrice = latest.unit_cost;
    const previousPrice = previous ? previous.unit_cost : null;
    const firstPrice = first.unit_cost;
    const priceChange = previousPrice !== null ? latestPrice - previousPrice : null;
    const priceChangePercent = previousPrice !== null && previousPrice !== 0
      ? ((latestPrice - previousPrice) / previousPrice) * 100
      : null;
    const invoiceIds = new Set(sorted.map((r) => r.invoice_id));
    const vendorNames = [...new Set(sorted.map((r) => r.invoices?.vendors?.name).filter(Boolean))];
    const history = sorted.map((r) => ({
      date: r.invoices?.invoice_date,
      unitCost: r.unit_cost,
      vendor: r.invoices?.vendors?.name || "",
      invoiceId: r.invoice_id,
      invoiceNumber: r.invoices?.invoice_number || ""
    }));

    products.push({
      name: latest.product_name_raw,
      brand: latest.brand,
      size: latest.size,
      latestPrice,
      previousPrice,
      ...
      history,
      lastSeen: latest.invoices?.invoice_date,
      vendors: vendorNames
    });
  }
...
export async function getVendorHistory() {
  ...
  const { data, error } = await supabase
    .from("vendors")
    .select("id, name, invoices(id, invoice_number, invoice_date, invoice_total, parse_status, original_file_name, created_at, stores(name), invoice_line_items(quantity,total_cost))")
    .eq("organization_id", organizationId)
    .order("name");
  if (error) throw error;
  return data || [];
...
export async function createVendorFolder({ name }) {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  const cleanName = String(name || "").trim();
  if (cleanName.length < 2) throw Object.assign(new Error("Vendor name must be at least 2 characters."), { status: 400 });

  const { data, error } = await supabase
    .from("vendors")
    .upsert({ organization_id: organizationId, name: cleanName }, { onConflict: "organization_id,name" })
    .select("id, name, created_at")
    .single();
  if (error) throw error;
  return { vendor: { ...data, invoices: [] } };
}

export async function mergeVendorFolders({ targetVendorId, sourceVendorId }) {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  if (!targetVendorId || !sourceVendorId) throw Object.assign(new Error("Choose two vendors to merge."), { status: 400 });
  if (targetVendorId === sourceVendorId) throw Object.assign(new Error("Choose a different vendor to merge."), { status: 400 });

  const { data: vendors, error: vendorError } = await supabase
    .from("vendors")
    .select("id, name")
    .eq("organization_id", organizationId)
    .in("id", [targetVendorId, sourceVendorId]);
  if (vendorError) throw vendorError;
  if ((vendors || []).length !== 2) throw Object.assign(new Error("One of those vendor folders could not be found."), { status: 404 });
...
export async function deleteVendorFolder(vendorId) {
  const supabase = getSupabaseAdmin();
  const organizationId = await getActiveOrganizationId();
  if (!vendorId) throw Object.assign(new Error("Choose a vendor folder to delete."), { status: 400 });
  ...
  const { data: vendor, error: vendorError } = await supabase
    ...
    .maybeSingle();
  if (vendorError) throw vendorError;
  if (!vendor) throw Object.assign(new Error("Vendor folder could not be found."), { status: 404 });
...
function serializeProcessedOcr({ text, files, error = null }) {
