...
export async function runGoogleVisionPdfPages({ fileBuffer, maxPages = 5 }) {
  ...
  const pageChunks = chunk(pages, 5);
  const results = [];

  for (const pageChunk of pageChunks) {
    try {
      results.push(...await runGoogleVisionPdfPageChunk({ fileBuffer, pages: pageChunk }));
    } catch (error) {
      if (isNoPagesFoundError(error) && results.length) break;
      throw error;
    }
  }
