...
import { batchUploadStoragePath } from "@/lib/storagePaths";
import { getStorageBucket, getSupabaseAdmin } from "@/lib/supabaseAdmin";

export async function findBatchByFileHash(fileHash, { organizationId: providedOrganizationId = null } = {}) {
...
export async function logDuplicateBatchUpload({ existingBatch, fileName, organizationId: providedOrganizationId = null }) {
  const supabase = getSupabaseAdmin();
  const organizationId = providedOrganizationId || await getActiveOrganizationId();
  const payload = {
...
export async function createInvoiceBatch({ file, fileBuffer, fileHash, mimeType, maxPages = 10, plan = "free", organizationId: providedOrganizationId = null }) {
  const supabase = getSupabaseAdmin();
  const bucket = getStorageBucket();
  const organizationId = providedOrganizationId || await getActiveOrganizationId();
  const path = batchUploadStoragePath({
    organizationId,
    uploadGroup: crypto.randomUUID(),
    fileName: file.name
  });

  const upload = await supabase.storage.from(bucket).upload(path, fileBuffer, {
    contentType: mimeType || file.type || "application/pdf",
    upsert: false
  });
